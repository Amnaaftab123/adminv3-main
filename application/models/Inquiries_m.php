...
class Inquiries_m extends MY_Model
{
    ...
    function get_sell_inquiries(){
        ...
        $result = $this->db->query("SELECT se.id, se.user_name as name, 
    ...
    function get_sale($id){
        ...
        $result = $this->db->query("SELECT sr.id, sr.first_name, sr.phone_no, expected_price,
        sr.email_id, sr.created_date, sr.status, sr.message, 
        m.make_name as make_title, mo.model_name as model_title, sr.mileage, sr.year, t.trim_name FROM  tbl_sell_enquiry sr 
        LEFT JOIN tbl_makes m ON sr.make_id = m.make_id 
        LEFT JOIN tbl_model mo ON sr.model_id = mo.model_id 
        LEFT JOIN tbl_trim t ON sr.trim_id = t.trim_id where sr.id = '".$id."'
        ");
    ...
    function get_contact_inquiries(){

        $result = $this->db->query("SELECT * FROM tbl_contact_enq ORDER BY id DESC");
    ...
    function get_contact_inquiry($id){

        $result = $this->db->query("SELECT * FROM tbl_contact_enq where id = '".$id."'");
