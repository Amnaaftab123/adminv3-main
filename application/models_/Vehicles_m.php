...
class Vehicles_m extends MY_Model
{
    ...
    function get_vehicles($data)
    {
        ...
        if(!empty($data['search_keyword'])){
            $criteria .= ' AND (m.title like "%' . $data['search_keyword'] . '%" OR mo.title like "%' . $data['search_keyword'] . '%" OR v.description like "%' . $data['search_keyword'] . '%" OR t.title like "%' . $data['search_keyword'] . '%") '; 
        }
        ...
        $r = $this->db->query(
            "SELECT v.vehicle_id, v.vehicle_details as description, v.sold, v.mileage, v.image as thumb_image, v.price, v.featured, v.car_view_counter, (select image_address from tbl_vehicle_images where vehicle_id = v.vehicle_id order by is_main desc limit 1) as image,v.year_id as year,v.options, v.vehicle_status, v.featured,
            m.title as make_name, mo.title as model_name, t.title as transmission_name FROM tbl_vehicle v 
            LEFT JOIN tbl_makes m ON v.make_id = m.id 
            LEFT JOIN tbl_model mo ON v.model_id = mo.id
            LEFT JOIN tbl_transmission t ON v.transmission_id = t.id WHERE v.vehicle_id > 0  $criteria"
        );
