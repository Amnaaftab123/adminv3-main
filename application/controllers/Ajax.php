...
class Ajax extends CI_Controller
{
    ...
    function make_data()
    {
        ...
        foreach ($this->mu->get_makes() as $item) {
            $data[] = array(
                ...
                'model' => '<a href = "' . site_url('utilities/model/' . $item['make_id']) . '">' . $item['total_model'] . '</a>',
                ...
                'status' => '<div class="btn-icon-list btn-list">
                    ' . (($item['status'] == 1) ? '<button type="button" class="btn btn-icon btn-danger me-1" onclick = "change_make_status(' . $item['make_id'] . ',0);"><i class="far fa-arrow-alt-circle-down"></i></button>' : '<button type="button" class="btn btn-icon btn-success me-1"  onclick = "change_make_status(' . $item['make_id'] . ',1);"><i class="far fa-arrow-alt-circle-up"></i></button>') . '
                    <button type="button" class="btn btn-icon  btn-primary me-1" onclick = "load_make(' . $item['make_id'] . ')"><i class="fe fe-edit"></i></button>
                    </div>'
            );
            $counter++;
        }
    ...
    function model_data()
    {
        ...
        foreach ($this->mu->get_models() as $item) {
            $data[] = array(
                ...
                'make' => '<a href = "' . site_url('utilities/make/') . '">' . $item['make_name'] . '</a>',
                'trim' => $item['trim_total'],

                'status' => '<div class="btn-icon-list btn-list">
                    ' . (($item['status'] == 1) ? '<button type="button" class="btn btn-icon btn-danger me-1" onclick = "change_model_status(' . $item['model_id'] . ',0);"><i class="far fa-arrow-alt-circle-down"></i></button>' : '<button type="button" class="btn btn-icon btn-success me-1"  onclick = "change_model_status(' . $item['model_id'] . ',1);"><i class="far fa-arrow-alt-circle-up"></i></button>') . '
                    <button type="button" class="btn btn-icon  btn-primary me-1" onclick = "load_model(' . $item['model_id'] . ')"><i class="fe fe-edit"></i></button>
                    </div>'
            );
            $counter++;
        }
    ...
    function trim_data()
    {
        ...
        foreach ($this->mu->get_trims_with_details() as $item) {
            $data[] = array(
                'id' => $counter,
                'title' => $item['trim_name'],
                'make_title' => '<a href = "' . site_url('utilities/make/') . '">' . $item['make_name'] . '</a>',
                'model_title' => '<a href = "' . site_url('utilities/model/') . '">' . $item['model_name'] . '</a>',
                'status' => '<div class="btn-icon-list btn-list">
                    ' . (($item['status'] == 1) ? '<button type="button" class="btn btn-icon btn-danger me-1" onclick = "change_trim_status(' . $item['trim_id'] . ',0);"><i class="far fa-arrow-alt-circle-down"></i></button>' : '<button type="button" class="btn btn-icon btn-success me-1"  onclick = "change_trim_status(' . $item['trim_id'] . ',1);"><i class="far fa-arrow-alt-circle-up"></i></button>') . '
                    <button type="button" class="btn btn-icon  btn-primary me-1" onclick = "load_trim(' . $item['trim_id'] . ')"><i class="fe fe-edit"></i></button>
                    </div>'
            );
            $counter++;
        }
    ...
    function load_sell_inquiries()
    {
        ...
        foreach ($this->inquiries_m->get_sell_inquiries() as $item) {
            ...
            $data[] = array(
                'title' => '<a href="javascript:void(0)" onclick = "load_sell_inqury_details(' . $item['id'] . ')"><div class = "d-flex "><div class = "pd-0 tx-14" >' . (($item['readed'] == 0) ? '<strong>' . ucwords($item['name']) . '</strong>' : ucwords($item['name'])) . '</div></div>' . maxLen($item['comments'], 30) . '</a>'
            );
            $counter++;
        }
    ...
    function load_sell_inquiry_details()
    {

        $result = $this->inquiries_m->get_sale($this->input->post('id'));


        if ($result) {

            $s = '<div class="media-body">
            <div class="media-title  font-weight-bold mt-3">'.ucwords($result['first_name']).' <span class="tx-13 font-weight-semibold">( ' . $result['email_id'] . ' )</span></div>
            <p class="mb-0"><i class = "fa fa-phone "></i> '.$result['phone_no'].'</p>
            <small class="me-2">' . date("M d, Y H:i A",($result['created_date'])) . '</small>
            <div class="eamil-body mt-3">
                <p>'.$result['message'].'</p><hr />
                <table class = "table table-bordered ">
                    <thead>
                    <tr>
                        <th>Vehicle</th>
                        <th>Expected Price (AED)</th>
                        <th>Milage (KM)
                    </tr>
                    </thead>
                    <tbody>
    ...
    function add_vehicle(){
        ...
        $i = array(
            ...
            'commercial' => $this->input->post('a_commercial'),
            ...
            'year' => $this->input->post('a_select_year'),
            ...
            'transmission' => $this->input->post('a_select_transmission'),

            'specification' => $this->input->post('a_select_specification'),
            'vehicle_condition' => $this->input->post('a_car_condition'),
            'engine_size' => $this->input->post('a_select_size'),
            'horsepower' => $this->input->post('a_select_hourse'),
    ...
    function get_sale_requests()
    {
        $roles = $this->inquiries_m->get_sales_request();

        $data = [];
        foreach ($roles as $role) {
            $data[] = array(
                'id' => $role['id'],
                'full_name' => '<a href = "' . site_url('external/sales/' . $role['id']) . '">' . $role['full_name']  . ' <small>('.(($role['account_type'] == '0') ? 'Individual' : 'Company') .')</small>' . '</a>',
                'telephone' => $role['telephone'],
                'email' => $role['email'],
                'middle_man' => $role['mid_full_name'] == "" ? 'No' : 'Yes',
                'price' => $role['price'],
                'vehicle' => $role['make_title'] . ' ' . $role['model_title'] . ' ' .$role['trim_name'] . ' ' . $role['year'],
                'offered_price' => $role['offered_price'],
                'created_at' => $role['created_at'],
                'status' => ucwords($role['current_status']),
                'agent' => $role['first_name'] . ' ' . $role['last_name'],
                'action' => '<div class="btn-icon-list btn-list"></div>'
            );
        }
        echo json_encode(array(
            'data' => $data
    ...
    function load_contact_inquiries()
    {

        $data = [];
        $counter = 1;
        foreach ($this->inquiries_m->get_contact_inquiries() as $item) {
            $data[] = array(
                'title' => '<a href="javascript:void(0)" onclick = "load_contact_inquiry_details(' . $item['id'] . ')"><div class = "d-flex "><div class = "pd-0 tx-14" >' . (($item['status'] == 0) ? '<strong>' . ucwords($item['name']) . '</strong>' : ucwords($item['subject'])) . '</div><div class = "mg-l-auto tx-10 tx-primary">' . getRangeDateString(($item['created_date'])) . '</div></div>' . maxLen($item['comments'], 30) . '</a>',
            );
            $counter++;
        }
    ...
    function load_contact_inquiry_details()
    {

        $result = $this->inquiries_m->get_contact_inquiry($this->input->post('id'));

        if ($result) {

            $s = '<div class="media-body">
            <div class="media-title  font-weight-bold mt-3">'.ucwords($result['name']).' <span class="tx-13 font-weight-semibold">( ' . $result['email_id'] . ' )</span></div>
            <p class="mb-0"><i class = "fa fa-phone "></i> '.$result['phone_no'].'</p>
            <small class="me-2">' . date("M d, Y H:i A",($result['created_date'])) . '</small>
            <div class="eamil-body mt-3">
                <p>'.$result['comments'].'</p><hr />
